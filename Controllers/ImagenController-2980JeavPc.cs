    ...
    public class ImagenController : Controller
    {
        ...
        public IActionResult Index()
        {
            using (SqlConnection con = new(_contexto.Conexion))
            {
                ...
                using (SqlCommand cmd = new("JeaUsr.sp_listar_imagenes", con))
                {
                ...
                if (imagen.File != null && imagen.Nombre != null)
                {
                    using (Stream fs = imagen.File.OpenReadStream())
                    {
                        using (BinaryReader br = new(fs))
                        {
                            ...
                            using (SqlConnection con = new(_contexto.Conexion))
                            {
                                using (SqlCommand cmd = new("JeaUsr.sp_insertar_imagen", con))
                                {
                                    cmd.CommandType = CommandType.StoredProcedure;
        ...
        public IActionResult Editar(int id)
        {
            ...
            {
                Imagen registro = new();
                using (SqlCommand cmd = new("JeaUsr.sp_buscar_imagen", con))
                {
                    cmd.CommandType = CommandType.StoredProcedure;
        ...
        public IActionResult Editar(Imagen imagen)
        {
            try
            {
                using (SqlConnection con = new(_contexto.Conexion))
                {
                    ...
                    using (SqlCommand cmd = new("JeaUsr.sp_actualizar_imagen", con))
                    {
                        cmd.CommandType = CommandType.StoredProcedure;
        ...
        public IActionResult Eliminar(Imagen img)
        {
            using (SqlConnection con = new(_contexto.Conexion))
            {
                using (SqlCommand cmd = new("JeaUsr.sp_eliminar_imagen", con))
                {
                    cmd.CommandType = CommandType.StoredProcedure;
